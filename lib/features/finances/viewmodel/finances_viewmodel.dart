...
class FinancesViewModel extends ChangeNotifier {
  ...
  List<EventHistory> _eventHistory = [
    EventHistory(id: '1', title: 'Birthday Party', date: '01 July 2025', amount: 500.00),
    EventHistory(id: '2', title: 'Concert', date: '31 Jun 2025', amount: 200.00),
    EventHistory(id: '3', title: 'Conference', date: '30 Jun 2025', amount: 100.00),
    EventHistory(id: '4', title: 'Friendly Party', date: '29 July 2025', amount: 400.00),
  ];
    ...
    if (_boothFeeController.text.isNotEmpty) {
      ...
      try {
        ...
        if (response.statusCode == 200 || response.statusCode == 201) {
          ...
          // NEW: Add booth name for dropdown
  ...
  try {
    final response = await http.get(
      Uri.parse("$_baseurl/event/events/list/"),
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer $accessToken',
      },
    ...
    if (response.statusCode == 200) {
      ...
      debugPrint("Failed to load event history: ${response.statusCode}");
      debugPrint("Response body: ${response.body}");
    }
  } catch (e) {
    debugPrint("Error fetching event history: $e");
  }
}
...
Future<void> editSale({required String eventId, required double newAmount}) async {
  final accessToken = await TokenStorage.getAccessToken();
  try {
    final response = await http.put(
      Uri.parse("$_baseurl/finance/finance/edit/$eventId/"),
      headers: {
        "Content-Type": "application/json",
        if (accessToken != null) "Authorization": "Bearer $accessToken",
      },
      body: jsonEncode({"sales": newAmount}),
    );

    if (response.statusCode == 200 || response.statusCode == 201) {
      print("✅ Sale updated successfully: ${response.body}");
      _totalSales = newAmount;
      _netProfit = _totalSales - _totalExpenses;
      notifyListeners();
    } else {
...
Future<void> editExpense({required String eventId, required double newAmount}) async {
  final accessToken = await TokenStorage.getAccessToken();
  try {
    final response = await http.put(
      Uri.parse("$_baseurl/finance/finance/$eventId/edit-expense/"),
      headers: {
        "Content-Type": "application/json",
        if (accessToken != null) "Authorization": "Bearer $accessToken",
      },
      body: jsonEncode({"expence": newAmount}),
    );

    if (response.statusCode == 200 || response.statusCode == 201) {
      print("✅ Expense updated successfully: ${response.body}");
      // Update local state if needed
      _totalExpenses = newAmount;
      _netProfit = _totalSales - _totalExpenses;
      notifyListeners();
    } else {
